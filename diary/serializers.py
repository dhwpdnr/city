...
class PlanetDiaryCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        raise CustomValidationError(f"{type(plants)} {plants} {len(plants)} {plants[0]} {type(plants[0])}", )
        ...
        if plants:
            plants_str = plants[0].replace("[", "").replace("]", "").replae(" ", "")
            plants_arr = list(plants_str.split(","))
            for plant_id in plants_arr:
